...
describe.only('HandlerWrapper', () => {
    ...
    async function _import(): Promise<ImportResult> {
        ...
        //     'silent',
        // ];
        // const loggerMock = new ObjectMock()
        //     .addMock('configure', () => loggerMock.instance)
        //     .addMock('getLogger', () => loggerMock.__loggerInstance);
        // loggerMock.__loggerInstance = LOG_METHODS.reduce((result: Record<string,SinonSpy<any[], any>>, method) => {
        //     result[method] = spy();
        //     return result;
        // }, {});

        // const logManagerMock: ObjectMock<LogManager> =
        //     new ObjectMock<LogManager>({} as LogManager);
        // logManagerMock.addMock('getLogger', stub());
        // logManagerMock.addMock('configure', stub());

        // logManagerMock.addMock('configure',  stub().callsFake(() => {
        ...
        ); //.setMock('logger', { default: logManagerMock.ctor });
        ...
        // importHelper.setMock('logManager', { default: logManagerMock.ctor });
        return {
    ...
    describe('ctor()', async function () {
        // it('should throw an error if invoked without a valid appName', async function () {

        //     const { testTarget: TargetClass } = await _import();
        //     const manager = new TargetClass('handler-wrapper');

        //     const message = 'Invalid appName (arg #1)';
        //     const inputs = _testValues.allButString('') as string[];

        //     inputs.forEach((appName:string) => {
        //         const wrapper = () => {
        //             return new HandlerWrapper(appName);
        //         };
        ...
        (_testValues.allButString('') as string[]).forEach(
            (appName: string) => {
                it(`should throw an error if invoked without a valid name (value=${appName})`, async function () {
                    const { testTarget: TargetClass } = await _import();
                    const message = 'Invalid appName (arg #1)';
        ...
        //     console.log(logManagerMock.mocks);

        //     await instance.wrap(() => {}, 'sample');

        //     console.log(logManagerMock.mocks);

        //     expect(logManagerMock.ctor).to.have.been.calledOnce;
        //     // expect(configureMethod.stub).to.have.been.calledOnce;
        //     // expect(enableMockMethod.stub.args[0][0]).to.equal(appName);
        //     // expect(enableMockMethod.stub.args[0][1]).to.deep.equal({
        //     //     level: 'info',
        //     //     extreme: false,
        //     // });
        // });
